...
class ScTuluCampaign {
  ...
  async loadCommonDataApi() {
    ...
    try {
      ...
      jsonData = await this.fetchApiData(
        "http://127.0.0.1:5501/json/updated/tradeup.json"
      );
  ...
  generateLevelUp() {
    ...
    if (articleId && articleImg && articleModal) {
      ...
      if (levelUpNextTask) {
        ...
        const taskValue = that.ScTuluCamp.querySelector(
          ".sc-tulu-camp-product-card-level-up .sc-tulu-camp-product-card__task-value"
        );
        if (taskValue) {
  ...
  async getStartedModal() {
    ...
    try {
      let jsonData;
      jsonData = await this.fetchApiData(
        "http://127.0.0.1:5501/json/updated/rewards_registration.json"
      );
  ...
  async loadArticleDataApi() {
    ...
    try {
      ...
      let jsonData;
      jsonData = await this.fetchApiData(
        "http://127.0.0.1:5501/json/updated/levelup.json"
      );
