...
class ScMeganav {
  init() {
    ...
    //Country clicked
    let country = document.querySelectorAll('.sc-meganav__country-btn');
    if (country.length) {
    ...
    document.addEventListener('click', function(event) {
      ...
      if (closeBtn) {
        if (event.target.closest('.sc-meganav .sc-modal .sc-modal__close-button')) {
          if (document.querySelector('.sc-meganav .sc-modal--show')) {
            document.querySelector('.sc-meganav .sc-modal').classList.remove('sc-modal--show');
            let countryBtn = document.querySelector('.sc-meganav .sc-meganav__country-btn');
      ...
      let clickModal = document.querySelector('.sc-meganav .sc-modal');
      let clickContent = document.querySelector('.sc-meganav .sc-modal .sc-modal__content');
      let clickClose = document.querySelector('.sc-meganav .sc-modal .sc-modal__close');
      if (clickModal && clickContent && clickClose) {
        let isClickInside1 = clickModal.contains(event.target);
        let isClickInside2 = event.target.closest('.sc-meganav .sc-modal .sc-modal__content');
        let isClickInside3 = event.target.closest('.sc-meganav .sc-modal .sc-modal__close');
        that.clickedOutsideModal(isClickInside1, isClickInside2, isClickInside3);
      }
    });

    //Country Search
    let cntSearch = document.querySelector('.sc-meganav .sc-country__input');
    if (cntSearch) {
      let countryList = [];
      let allCntList = '';
      //Create country list
   ...
   */
  activateDropDown() {
    const btn = document.querySelectorAll('.sc-meganav-account-dropdown__btn');
    const containers = document.querySelectorAll('.sc-meganav-account-dropdown__container');
    btn.forEach((item, index) => {
      item.addEventListener('click', event => {
        ...
        btn.forEach(btnItems => {
          btnItems.classList.remove('sc-meganav-account-dropdown__btn--active');
        });
        containers.forEach(container => {
          container.classList.add('hide');
        });
        // Add the active class to the clicked button
        item.classList.add('sc-meganav-account-dropdown__btn--active');
  ...
  navFontWeightFixes(urlList) {
    ...
    if (url.includes('/cn/en/')) {
      let navBtn = document.querySelectorAll('.sc-meganav-drop__btn,.sc-meganav-drop__promo-title');
      if (navBtn.length) {
        navBtn.forEach(e => {
   ...
   **/
  legacyImageFixes() {
    let elem = document.querySelectorAll('.sc-meganav-drop__img');
    if (elem.length) {
      elem.forEach(e => {
  ...
  accessibility() {
    document.body.addEventListener('keyup', function(event) {
      ...
      if (event.key && event.key.toLowerCase() === 'enter') {
        //Left topNav start
        let topNav = event.target.closest('.sc-meganav__top-btn');
        if (topNav) {
          ...
          if (
            ...
            siblingClass.indexOf('sc-meganav__personal-list--show') !== -1
          ) {
            event.target
              .closest('.sc-meganav__top-btn')
              .nextSibling.nextSibling.classList.remove('sc-meganav__personal-list--show');
          } else {
            event.target
              .closest('.sc-meganav__top-btn')
              .nextSibling.nextSibling.classList.add('sc-meganav__personal-list--show');
          }
        }

        //MegaNav start
        let megaNav = event.target.closest('.sc-meganav__menu');
        if (megaNav) {
          let siblingClass = event.target.closest('.sc-meganav__item--has-meganav').className;

          if (
            typeof siblingClass === 'string' &&
            typeof siblingClass.includes !== 'undefined' &&
            siblingClass.indexOf('sc-meganav__item--show') !== -1
          ) {
            event.target
              .closest('.sc-meganav__menu')
              .nextSibling.nextSibling.classList.remove('sc-meganav__item--show');
          } else {
            event.target
        ...
        //Login start
        let loginBtn = event.target.closest('.sc-meganav__login-btn');
        if (loginBtn) {
          let loginDrop = document.querySelector('.sc-meganav-login-drop');
          let siblingClass = loginDrop.className;
          if (
            typeof siblingClass === 'string' &&
            typeof siblingClass.includes !== 'undefined' &&
            siblingClass.indexOf('sc-meganav-login-drop--show') !== -1
          ) {
            loginDrop.classList.remove('sc-meganav-login-drop--show');
          } else {
        ...
        //Open an Account
        const meganavAccount = event.target.closest('.sc-meganav__account-btn');
        if (meganavAccount) {
          const meganavAccountDrop = document.querySelector('.sc-meganav-account-dropdown');
          const siblingClass = meganavAccountDrop.className;
          if (
            typeof siblingClass === 'string' &&
            typeof siblingClass.includes !== 'undefined' &&
            siblingClass.indexOf('sc-meganav-account-dropdown--show') !== -1
          ) {
            meganavAccountDrop.classList.remove('sc-meganav-account-dropdown--show');
            meganavAccount.classList.remove('sc-meganav__account-btn--show');
          } else {
            meganavAccountDrop.classList.add('sc-meganav-account-dropdown--show');
            meganavAccount.classList.add('sc-meganav__account-btn--show');
          }
        }
      ...
      if (event.key && event.key.toLowerCase() === 'tab') {
        //Left topNav start
        let topNav = event.target.closest('.sc-meganav__top-btn');
        let personalList = event.target.closest('.sc-meganav__personal-list');
        if (!topNav && !personalList) {
          //when outside of personal list
          let personalListShow = document.querySelectorAll('.sc-meganav__personal-list');
          if (personalListShow.length) {
            for (let i = 0; i < personalListShow.length; i++) {
              personalListShow[i].classList.remove('sc-meganav__personal-list--show');
            }
          }
        } else if (topNav) {
          //Remove if selected next top nav button
          let siblingClass = event.target.closest('.sc-meganav__top-btn').nextSibling.nextSibling
            .className;
          if (siblingClass.indexOf('sc-meganav__personal-list--show') == -1) {
            let personalListShow = document.querySelectorAll('.sc-meganav__personal-list');
            if (personalListShow.length) {
              for (let i = 0; i < personalListShow.length; i++) {
                personalListShow[i].classList.remove('sc-meganav__personal-list--show');
        ...
        //MegaNav start
        let megaNav = event.target.closest('.sc-meganav__menu');
        let wrapperList = event.target.closest('.sc-meganav__item--has-meganav');
        if (!megaNav && !wrapperList) {
          //when outside of personal list
          let wrapperListShow = document.querySelectorAll('.sc-meganav__item--has-meganav');
          if (wrapperListShow.length) {
            for (let i = 0; i < wrapperListShow.length; i++) {
              wrapperListShow[i].classList.remove('sc-meganav__item--show');
            }
          }
        } else if (megaNav) {
          //Remove if selected next top nav button
          if (wrapperList.className.indexOf('sc-meganav__item--show') == -1) {
            let wrapperListShow = document.querySelectorAll('.sc-meganav__item--has-meganav');
            if (wrapperListShow.length) {
              for (let i = 0; i < wrapperListShow.length; i++) {
                wrapperListShow[i].classList.remove('sc-meganav__item--show');
        ...
        //Login Start
        let loginBtn = event.target.closest('.sc-meganav__login-btn');
        let loginList = event.target.closest('.sc-meganav-login-drop');
        if (!topNav && !loginList) {
          //when outside of personal list
          let loginListShow = document.querySelectorAll('.sc-meganav-login-drop');
          if (loginListShow.length) {
            for (let i = 0; i < loginListShow.length; i++) {
              loginListShow[i].classList.remove('sc-meganav-login-drop--show');
            }
          }
        } else if (loginBtn) {
          //Remove if selected next top nav button
          let siblingClass = document.querySelector('.sc-meganav-login-drop').className;
          if (siblingClass.indexOf('sc-meganav-login-drop--show') == -1) {
            let loginListShow = document.querySelectorAll('.sc-meganav-login-drop');
            if (loginListShow.length) {
              for (let i = 0; i < loginListShow.length; i++) {
                loginListShow[i].classList.remove('sc-meganav-login-drop--show');
        ...
        // Open an Account
        const openAccountBtn = event.target.closest('.sc-meganav__account-btn');
        const openAccountDrop = event.target.closest('.sc-meganav-account-dropdown');
        if (!openAccountBtn && !openAccountDrop) {
          const openAccountShow = document.querySelectorAll('.sc-meganav-account-dropdown--show');
          if (openAccountShow.length) {
            for (let i = 0; i < openAccountShow.length; i++) {
              openAccountShow[i].classList.remove('sc-meganav-account-dropdown--show');
            }
          }
        } else if (openAccountBtn) {
          const siblingClass = document.querySelector('.sc-meganav-account-dropdown').className;
          if (siblingClass.indexOf('sc-meganav-account-dropdown--show') === -1) {
            const openAccountShow = document.querySelectorAll('.sc-meganav-account-dropdown');
            if (openAccountShow.length) {
              for (let i = 0; i < openAccountShow.length; i++) {
                openAccountShow[i].classList.remove('sc-meganav-account-dropdown--show');
              }
    ...
    document.querySelector('body').addEventListener('mouseover', function(event) {
      ...
      let personalList = event.target.closest('.sc-meganav__personal-list--show');
      if (!personalList) {
        let personalShow = document.querySelectorAll('.sc-meganav__personal-list--show');
        if (personalShow.length) {
          for (let i = 0; i < personalShow.length; i++) {
            personalShow[i].classList.remove('sc-meganav__personal-list--show');
          }
        }
      }

      //Remove megaNav class
      let scNav = event.target.closest('.sc-meganav');
      if (!scNav) {
        let wrapperShow = document.querySelectorAll('.sc-meganav__mgnv-wrapper--show');
        if (wrapperShow.length) {
          for (let i = 0; i < wrapperShow.length; i++) {
            wrapperShow[i].classList.remove('sc-meganav__mgnv-wrapper--show');
          }
        }
      }
    });
  ...
  browserDesktop() {
    ...
    let navItems = document.querySelectorAll('.sc-meganav__item');
    if (navItems.length) {
  ...
  removeDesktopClasses() {
    ...
    let scNav = document.querySelector('.sc-meganav');
    if (scNav) {
  ...
  removeMobileClasses() {
    ...
    if (document.querySelector('.sc-meganav__close-button')) {
      document.querySelector('.sc-meganav__close-button').classList.remove('sc-menu-visible');
    }
    let navPersonal = document.querySelector('.sc-meganav__personal-list');
    if (navPersonal) {
      navPersonal.classList.remove('sc-menu-visible');
    }
    let mgnvWrapperList = document.querySelectorAll('.sc-meganav__mgnv-wrapper');
  ...
  browserMobile() {
    //Click on hamburger
    let scNav = document.querySelector('.sc-meganav');
    let hamburger = document.querySelector('.sc-meganav__hamburger');
    const loginDrop = document.querySelector('.sc-meganav-login-drop');
    const loginClose = document.querySelector('.sc-meganav__login-close');
    const megaNavClose = document.querySelector('.sc-meganav__close-button');
    const megaNavTop = document.querySelector('.sc-meganav__top');
    const megaNavList = document.querySelector('.sc-meganav__list');
    ...
    let topBtn = document.querySelectorAll('.sc-meganav__top-btn');
    if (topBtn.length) {
      topBtn.forEach(function(el) {
        el.addEventListener('click', function(event) {
          let siblingClass = event.target
            .closest('.sc-meganav__top-in')
            .querySelector('.sc-meganav__personal-list').className;
          let personalList = document.querySelectorAll('.sc-meganav__personal-list');
          if (
            typeof siblingClass === 'string' &&
            typeof siblingClass !== 'undefined' &&
    ...
    let navMenu = document.querySelectorAll('.sc-meganav__menu');
    if (navMenu.length > 0) {
      navMenu.forEach(function(el) {
        el.addEventListener('click', function(event) {
          const closest = event.target.closest('.sc-meganav__item');
          if (closest) {
            const siblingClass = closest.querySelector('.sc-meganav__mgnv-wrapper');
            if (siblingClass) {
    ...
    //close dropdown back
    let dropDownList = document.querySelectorAll('.sc-meganav-drop__back');
    ...
    //click login button
    let loginBtn = document.querySelector('.sc-meganav__login-btn');
    if (loginBtn) {
  ...
  stickyNavbar(scrollTop) {
    let navContainer = document.querySelector('.sc-meganav');
    if (navContainer) {
      scrollTop >= 30
  ...
  showModal() {
    if (document.querySelector('.sc-meganav .sc-modal')) {
      document.querySelector('.sc-meganav .sc-modal').classList.add('sc-modal--show');
      let modalContent = document.querySelector('.sc-meganav .sc-modal .sc-modal__content');
  ...
  languageSelector(urlList, countryLanguages) {
    ...
    let languageList = document.querySelectorAll('.sc-language-selector .sc-meganav__lang-link');
      ...
      for (let i = 0; i < personalList.length; i++) {
        ...
        if (title) {
          ...
          if (pageSegment === title && href) {
            ...
            const topBtn = document.querySelector('.sc-meganav__top-in .sc-meganav__top-btn');
            if (topBtn) {
              topBtn.innerHTML = trim(titleText);
            }
            const activeCls = document.querySelector('.sc-meganav__personal-box-link--active');
            if (activeCls) {
              activeCls.classList.remove('sc-meganav__personal-box-link--active');
            }
            personalList[i].classList.add('sc-meganav__personal-box-link--active');
          }
        }
      }
    }
      ...
      for (let i = 0; i < personalListDropDown.length; i++) {
        ...
        if (title) {
          ...
          if (pageSegment === title && href) {
            ...
            const topBtn = document.querySelector('.sc-meganav__priority-drop-btn');
            if (topBtn) {
              topBtn.innerHTML = trim(titleText);
            }
          }
  ...
  updateOpenAccountNavigation() {
    ...
    const accountBtn = document.querySelector('.sc-meganav__account-btn');
    const openAccount = document.querySelector('.sc-meganav__open-account');
  ...
  prioritySaadiqIslamic(pageContext) {
    ...
    let logo = document.querySelector('.sc-meganav .sc-meganav__logo');
    ...
    if (
      ...
      let logoLink = document.querySelector('.sc-meganav__logo-link');
      if (logoLink) {
        logoLink.innerHTML = `
      ...
      let linkNew = document.querySelector('.sc-meganav__logo-link-new');
      if (linkNew) {
        linkNew.innerHTML = `
         <div class="sc-meganav__saadiq-logo-text"></div>
         <div class="sc-meganav__saadiq-logo"></div>
    ...
    let priorityLogoStyles = `.sc-meganav__logo .sc-meganav__logo-link{background-image:url(https://av.sc.com/assets/global/icons/svg/standard-chartered-logo-priority-white.svg) !important}`;

    /* eslint-disable no-undef */
    let priorityPrivateLogoStyle = `.sc-meganav__logo .sc-meganav__logo-link{background-image:url(https://av.sc.com/assets/global/icons/svg/standard-chartered-logo-priority-private-white.svg) !important}`;

    /* eslint-disable no-undef */
    let privateBankingLogoStyle = `.sc-meganav__logo .sc-meganav__logo-link{background-image:url(https://av.sc.com/assets/global/icons/svg/standard-chartered-logo-private-bank-white.svg) !important}`;

    if (
      document.querySelector('.sc-meganav') &&
      !document.querySelector('.sc-meganav').classList.contains('sc-meganav--dark')
    ) {
      /* eslint-disable no-undef */
      priorityLogoStyles = `.sc-meganav__logo .sc-meganav__logo-link{background-image:url(https://av.sc.com/assets/global/images/components/header/standard-chartered-logo-priority.svg) !important}`;

      /* eslint-disable no-undef */
      priorityPrivateLogoStyle = `.sc-meganav__logo .sc-meganav__logo-link{background-image:url(https://av.sc.com/assets/global/images/components/header/standard-chartered-logo-priority-private.svg) !important}`;

      /* eslint-disable no-undef */
      privateBankingLogoStyle = `.sc-meganav__logo .sc-meganav__logo-link{background-image:url(https://av.sc.com/assets/global/images/components/header/standard-chartered-logo-private-bank.svg) !important}`;
    }
