 ...
 */
const isInternalDomain = (domain, ctaUrl) => {
  if (isForm(ctaUrl)) {
    return true;
  }
...
const extractQueryParams = (queryParams = [], excludeParams = false) => {
  ...
  for (let [key, value] of params.entries()) {
    const shouldInclude = excludeParams
 ...
 */
const getCtaContext = (closest) => {
  const context =
    closest.getAttribute("data-context") ??
    closest.getAttribute("title") ??
    closest.innerText ??
    closest.textContent ??
...
const handlePersistentBarScroll = () => {
  ...
  if (pBarSkip) {
    ...
    if (selectedElement) {
      ...
      skipHeight = getOuterHeight(selectedElement);
      if (windowWidth >= 768) {
        //Ignore menu height
        skipFromTop -= 70;
      }
  ...
  const browser = {
    ...
    searchString(data) {
      for (let i = 0; i < data.length; i++) {
        ...
        const subString = data[i].subString;
 ...
 */
const getFormattedNumber = (num, language = "en") => {
