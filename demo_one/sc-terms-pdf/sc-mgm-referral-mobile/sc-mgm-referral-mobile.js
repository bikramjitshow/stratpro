...
class ScMgmReferralMobile {
  init() {
    ...
    that.codeValue = ScCommonMethods.getQueryParam(
    ...
    document.body.addEventListener('click', function(event) {
      if (modalOpen) {
        ...
        let tcModal = document.querySelector('.m-text-content').getAttribute('data-term-modal');
        if (
        ...
        if (window.tcmObject.termsAccepted && event.target.classList.contains('sc-mgm-refer-tc')) {
  ...
  checkAndUpdateDate() {
    ...
    if (!that.isValidUTCTimestamp(storedDateTimestamp)) {
      console.warn('Invalid timestamp in localStorage, resetting to the new value.');
