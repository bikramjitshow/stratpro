...
const urlBreakdown = (currentURL) => {
  console.log("urlBreakdown--", currentURL);
  ...
  if (
    parts.length <= 4 &&
    scCommonConstants.LANGUAGES.indexOf(pageURL.slug) !== -1
  ) {
...
const calcElementLocation = (target) => {
  ...
  if (target.closest(".sc-persistent-bar")) {
  ...
  if (target.closest(".sc-banner")) {
    return "banner";
  }
  if (target.closest(".sc-footer")) {
    return "footer";
  }
  if (target.closest(".sc-banner-przn-block")) {
    return "banner-widget";
  }

  if (target.closest(".sc-quick-tools")) {
    return "quick-link";
  }
  if (target.closest(".sc-banner-slider__thumbnail")) {
    return "banner-tab";
  }
  if (target.closest(".sc-filter")) {
    return "tab";
  }
  if (target.closest(".sc-digital-guide .splide")) {
    return "tab";
  }
  if (target.closest(".sc-benefits-slider-tab__tab")) {
    return "tab";
  }
...
const handleAnalyticsCTA = (ev, targetClass = "", ctaObj = {}) => {
  ...
  let customLinkText =
    closest.getAttribute("title") || closest.innerText || closest.textContent;
  if (customLinkText) {
    customLinkText = customLinkText.trim().toLowerCase();
  }
  let { ctaType, ctaPosition, customLinkRegion } = ctaObj;
  ...
  if (!customLinkRegion) {
    customLinkRegion =
      getHorizontalPosition(ev.clientX) + " " + calcElementLocation(ev.target);
  }
...
const shouldCarryTrackingParams = (url) => {
  ...
  if (
    ...
    for (
      var j = 0;
      j < scCommonConstants.ALLOWABLE_QUERYSTRING["partner-urls"].length;
      j++
    ) {
...
const applyCookieParams = (
  ...
  var allowableQueryStringOthers =
    scCommonConstants.ALLOWABLE_QUERYSTRING.others;
  var swap_map = {};
  if ($swapping_attrs !== "") {
...
const getUrlParamName = (paramName) => {
  ...
  for (var i = 0; i < scCommonConstants.ALLOWABLE_QUERYSTRING.scb.length; i++) {
    if (
      ...
      scCommonConstants.ALLOWABLE_QUERYSTRING.scb[i].toLowerCase()
    ) {
      index = i;
      break;
    }
