...
class ScMeganav {
  init() {
    ...
    document.addEventListener("click", function (event) {
      ...
      let clickContent = document.querySelector(
        ".sc-meganav .sc-modal .sc-modal__content"
      );
      let clickClose = document.querySelector(
      ...
      if (clickModal && clickContent && clickClose) {
        ...
        let isClickInside2 = event.target.closest(
          ".sc-meganav .sc-modal .sc-modal__content"
        );
        let isClickInside3 = event.target.closest(
          ".sc-meganav .sc-modal .sc-modal__close"
        );
  ...
  navFontWeightFixes(urlList) {
    ...
    if (url.includes("/cn/en/")) {
      let navBtn = document.querySelectorAll(
    ...
    if (anchorList.length) {
      anchorList.forEach(function (evt) {
        evt.addEventListener("click", function (event) {
          ...
          if (
            anchorId &&
            (anchorId.substr(0, 1) == "." || anchorId.substr(0, 1) == "#")
          ) {
  ...
  accessibility() {
    document.body.addEventListener("keyup", function (event) {
      ...
      if (event.key && event.key.toLowerCase() === "enter") {
        ...
        if (megaNav) {
          let siblingClass = event.target.closest(
        ...
        //Link Hover start
        let linkHover = event.target.closest(
      ...
      if (event.key && event.key.toLowerCase() === "tab") {
        ...
        if (!topNav && !personalList) {
          ...
          let personalListShow = document.querySelectorAll(
          ...
          let siblingClass = event.target.closest(".sc-meganav__top-btn")
            .nextSibling.nextSibling.className;
        ...
        //MegaNav start
        let megaNav = event.target.closest(".sc-meganav__menu");
        let wrapperList = event.target.closest(
          ".sc-meganav__item--has-meganav"
        );
        if (!megaNav && !wrapperList) {
          //when outside of personal list
          let wrapperListShow = document.querySelectorAll(
            ".sc-meganav__item--has-meganav"
          );
        ...
        if (!topNav && !loginList) {
          //when outside of personal list
          let loginListShow = document.querySelectorAll(
          ...
          //Remove if selected next top nav button
          let siblingClass = document.querySelector(
            ".sc-meganav-login-drop"
          ).className;
          if (siblingClass.indexOf("sc-meganav-login-drop--show") == -1) {
            ...
            if (loginListShow.length) {
              for (let i = 0; i < loginListShow.length; i++) {
                loginListShow[i].classList.remove(
    ...
    document
      ...
      .addEventListener("mouseover", function (event) {
        ...
        let personalList = event.target.closest(
          ".sc-meganav__personal-list--show"
        );
        if (!personalList) {
          let personalShow = document.querySelectorAll(
            ".sc-meganav__personal-list--show"
          );
        ...
        if (!scNav) {
          let wrapperShow = document.querySelectorAll(
  ...
  browserDesktop() {
    ...
    let linkHover = document.querySelectorAll(
      ".sc-mgdrop__list-item-link--hover"
    );
    if (linkHover.length) {
  ...
  calculateHeight(dropdownItems) {
    ...
    dropdownHeight =
      dropdownHeight * Math.ceil(dropdownItems.length / 3) + shortFallHeight;
    let mgDropRight = document.querySelector(".sc-mgdrop__right");
    if (mgDropRight) {
  ...
  removeMobileClasses() {
    ...
    let mgnvWrapperList = document.querySelectorAll(
  ...
  browserMobile() {
      ...
      topBtn.forEach(function (el) {
        el.addEventListener("click", function (event) {
          let siblingClass = event.target
            .closest(".sc-meganav__top-in")
            .querySelector(".sc-meganav__personal-list").className;
          let personalList = document.querySelectorAll(
            ".sc-meganav__personal-list"
          );
          if (
            typeof siblingClass === "string" &&
    ...
    if (navMenu.length > 0) {
      navMenu.forEach(function (el) {
        el.addEventListener("click", function (event) {
          ...
          if (closest) {
            const siblingClass = closest.querySelector(
              ".sc-meganav__mgnv-wrapper"
            );
            if (siblingClass) {
    ...
    if (dropDownList.length > 0) {
      dropDownList.forEach(function (el) {
        el.addEventListener("click", function () {
          ...
          let itemWrapper = document.querySelectorAll(
            ".sc-mgdrop__item-wrapper"
          );
          let oneBack = true;
          if (itemWrapper) {
    ...
    //Fixed height
    let itemHover = document.querySelectorAll(
      ".sc-mgdrop__list-item-link--hover"
    );
    if (itemHover.length) {
  ...
  languageSelector(urlList, countryLanguages) {
    console.log({ urlList, countryLanguages });
    if (!urlList) return;
    let URL = urlList.domain + urlList.pathname;

    //Header language selector
    let languageList = document.querySelectorAll(
      ...
      languageList.forEach(function (el) {
        el.addEventListener("click", function (event) {
          ...
          let selectedLanguage = document
            ...
            .toLowerCase();
          let clickedLanguage = event.target
          ...
          if (country == "hk") {
            ...
            if (clickedLanguage == "tzh") {
              ...
              clickedLanguage == countryLanguages[getCurrentCountry()].default
            ) {
  ...
  menuSlug() {
    ...
    if (personalList.length) {
      for (let i = 0; i < personalList.length; i++) {
        ...
        if (title) {
          const selectedTitle = personalList[i].getAttribute(
            "data-selected-title"
          );
          ...
          if (pageSegment === title && href) {
            const titleText =
              personalList[i].innerText || personalList[i].textContent;
            ...
            const activeCls = document.querySelector(
    ...
    if (personalListDropDown.length) {
      for (let i = 0; i < personalListDropDown.length; i++) {
        ...
        if (title) {
          const selectedTitle = personalListDropDown[i].getAttribute(
            "data-selected-title"
          );
          ...
          if (pageSegment === title && href) {
            const titleText =
              personalListDropDown[i].innerText ||
              personalListDropDown[i].textContent;
