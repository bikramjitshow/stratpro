...
import {
  ...
  trim,
  handleAnalyticsCTA,
} from "../scripts/sc-common-methods-2";
...
class ScMeganav {
  init() {
    ...
    document.addEventListener("click", function (event) {
      ...
      let clickContent = document.querySelector(
        ".sc-nav .sc-modal .sc-modal__content"
      );
      let clickClose = document.querySelector(
        ".sc-nav .sc-modal .sc-modal__close"
      );
  ...
  activateDropDown() {
    ...
    const containers = document.querySelectorAll(
      ".sc-nav-account-dropdown__container"
    );
  ...
  navFontWeightFixes(urlList) {
    ...
    if (url.includes("/cn/en/")) {
      let navBtn = document.querySelectorAll(
        ".sc-mgdrop__btn,.sc-mgdrop__promo-title"
      );
      if (navBtn.length) {
        navBtn.forEach((e) => {
    ...
    document.body.addEventListener("keyup", function (event) {
      ...
      if (event.key && event.key.toLowerCase() === "enter") {
        ...
        if (megaNav) {
          ...
          if (
            ...
            event.target
              .closest(".sc-nav__item--has-meganav")
              .classList.add("sc-nav__item--show");
          }
        }
      ...
      if (event.key && event.key.toLowerCase() === "tab") {
        ...
        if (!topNav && !personalList) {
          ...
          let siblingClass =
            event.target.closest(".sc-nav__top-btn").nextSibling.nextSibling
              .className;
  ...
  removeMobileClasses() {
    ...
    if (document.querySelector(".sc-nav__close-button")) {
      document
        .querySelector(".sc-nav__close-button")
  ...
  languageSelector(urlList, countryLanguages) {
    ...
    let languageList = document.querySelectorAll(
      ".c-language-selector .sc-nav__lang-link"
    );
