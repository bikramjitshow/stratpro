...
class ScTulu {
  init() {
    ...
    that.tabs.forEach(tab => {
      tab.addEventListener('click', event => {
        ...
        const closestPdt = event.target.closest('.sc-tulu-camp-product-card__main');
        if (closestPdt) {
          handleAnalyticsCTA(event, '', {
            context: trim(closestPdt.querySelector('.sc-tulu-camp-product-card__title').innerText),
            ctaType: 'button',
            xLinkRegion: 'middle',
            ctaPosition: 'middle'
          });
  ...
  getStartedModal() {
    ...
    const checkboxes = that.ScTuluCamp.querySelectorAll(".sc-get-started input[type='checkbox']");
  ...
  generateHTMLCode(selector, data, type = 'design-1') {
    ...
    if (type === 'design-1') {
      ...
      let rewardValue = 0;
      let rewardCountList = [];
      const progressSections = that.ScTuluCamp.querySelectorAll('.sc-tulu-camp-progress__inner');
      ...
      if (lastElement) {
        const lastElementList = lastElement.querySelectorAll('li.sc-tulu-camp-progress__section');

        rewardCountList.forEach((value, index) => {
          const completedSections =
            updatedProgressSections[index]?.querySelectorAll('li.sc-tulu-camp-progress__section')
              .length || 0;
  ...
  async loadConsolidateApi() {
    ...
    for (const el of consolidates) {
      try {
        const data = await this.fetchApiData(el.getAttribute('data-consolidate'));
        let value = Number(data.Fields?.[0]?.Value) || 0;
        totalMiles += value;
        value = convertNumbers(that.ScTuluCamp, value);

        const accumulated = el.querySelector('.sc-tulu-camp-product-card__progress-total strong');

        if (accumulated) {
          let accumulatedValue = accumulated.innerText.trim().replace(/[^0-9.]/g, '');
          ...
          accumulated.innerText = convertNumbers(that.ScTuluCamp, accumulatedValue.toString());
        }
        ...
        const childClass = that.ScTuluCamp.querySelector('.' + el.getAttribute('data-child-class'));
        if (childClass) {
    ...
    const milesSelector = that.ScTuluCamp?.querySelector('.sc-cvp__middle strong');
    if (milesSelector) {
      milesSelector.innerText = convertNumbers(that.ScTuluCamp, totalMiles);
    }
    const totalAccumulated = that.ScTuluCamp?.querySelector('.sc-tulu-camp-milescard__desc strong');
    if (totalAccumulated) {
      totalAccumulated.innerText = convertNumbers(that.ScTuluCamp, totalMilesAccumulated);
    }
  }
  ...
  handleTabs(targetId) {
    ...
    that.tabs.forEach(t => t.classList.remove('sc-tulu-camp-tab__head-button--active'));
    that.contents.forEach(c => c.classList.remove('sc-tulu-camp-tab__content-item--active'));

    // Add active class to clicked tab and corresponding content
    const activeTabs = that.ScTuluCamp.querySelectorAll(`[data-tab-btn-id="${targetId}"]`);
  ...
  handleModal() {
    ...
    that.ScTuluCamp.addEventListener('click', event => {
      ...
      const articleAnchor = event.target.closest('a.sc-tulu-camp-timeline__cvp-title');
      if (modal) {
        event.preventDefault();
        ...
        if (selector) {
          ...
          if (selectedModal) {
            ...
            if (selectedModal.closest('.sc-questionnaire')) {
              ...
              const btn = selectedModal.querySelector('.sc-tulu-camp-modal__footer-btn');
              if (btn) {
        ...
        if (anchorElement) {
          const modalSelector = anchorElement.getAttribute('data-modal-selector');
        ...
        if (buttonElement) {
          const modalSelector = buttonElement.getAttribute('data-modal-selector');
        ...
        //Next Task
        const nextTaskElement = event.target.closest('.sc-tulu-camp-product-card__bottom');
        if (nextTaskElement) {
          const title = trim(
            nextTaskElement
          ...
          Object.assign(ctaObj, {
            context:
              `${title} next task - ` +
              trim(
                nextTaskElement.querySelector('.sc-tulu-camp-product-card__task-desc').innerText
              ),
            ctaPosition: 'middle',
            xLinkRegion: 'middle'
          });
        ...
        //Timeline boxes
        const timeLineElement = event.target.closest('.sc-tulu-camp-wrapper-full__column');
        if (timeLineElement) {
          const diversifyTitle = timeLineElement.querySelector('.sc-tulu-camp-wrapper-full__title');
          if (diversifyTitle && diversifyTitle.getAttribute('data-context')) {
            //diversify section
            const title = trim(diversifyTitle.getAttribute('data-context'));
            const content = trim(
              timeLineElement.querySelector('.sc-tulu-camp-timeline__box-title').innerText
            );
            ...
            Object.assign(ctaObj, {
              context:
                `article test - ` +
                trim(timeLineElement.querySelector('.sc-tulu-camp-timeline__box-title').innerText),
              ctaPosition: 'middle',
              xLinkRegion: 'middle'
            });
          }
        ...
        if (anchorElement ?? buttonElement ?? nextTaskElement ?? timeLineElement) {
          handleAnalyticsCTA(
            event,
            anchorElement ?? buttonElement ?? nextTaskElement ?? timeLineElement,
            ctaObj
          );
        }
    ...
    //Close Modal
    const modalCloses = that.ScTuluCamp.querySelectorAll('.sc-tulu-camp-modal__close');
        ...
        modalClose.addEventListener('click', event => {
          if (!event.target.closest('.sc-get-started')) {
            ...
            if (that.parentModalId) {
              that.ScTuluCamp.querySelector(`.${that.parentModalId}`).classList.add('visible');
              that.parentModalId = '';
            } else {
              document.querySelector('html').classList.remove('sc-modal__no-scroll');
            }
    ...
    const downloads = that.ScTuluCamp.querySelectorAll('.sc-tulu-camp-modal__download');
